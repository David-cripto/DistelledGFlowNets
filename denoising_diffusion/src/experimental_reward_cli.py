...
from .experimental_reward import (
...
def _default_out_dir(dataset: str) -> Path:
    return Path("outputs") / "denoising_diffusion" / dataset / "experimental_reward_run"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train the experimental quadratic-offset reward model "
            "F(x, t) = NN(x, t) - 0.5 w(t) ||x||^2 on DDPM image transitions"
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="Number of data examples used in each experimental reward minibatch",
    )
...
def main() -> None:
    ...
    reward_config = ExperimentalRewardTrainConfig(
    ...
    reward_result = train_experimental_reward_model(
    ...
    print(f"Saved experimental reward run to: {out_dir}")
